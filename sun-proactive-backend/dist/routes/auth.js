...
export default async function authRoutes(server) {
    ...
    server.post('/login', async (request, reply) => {
        try {
            ...
            if (user.role === 'volunteer')
                redirect = '/volunteer';
            return { success: true, token, redirect, user: { id: user.id, role: user.role, isApproved: user.isApproved } };
        }
        catch (error) {
    ...
    server.post('/register', async (request, reply) => {
        try {
            ...
            const { email, password, role, name, skills, birthDate } = data;
            ...
            if (role === 'volunteer') {
                if (!birthDate)
                    return reply.status(400).send({ error: 'Укажите дату рождения' });
            ...
            if (role === 'volunteer') {
                let embeddingVector = Array.from({ length: 1536 }, () => Math.random() * 0.1);
                try {
                    const embedRes = await ai.embeddings.create({
                        model: 'text-embedding-3-small',
                        input: `Навыки волонтера: ${skills?.join(', ')}. Имя: ${name}`
                    });
